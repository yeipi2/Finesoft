...
public class EmailService : IEmailService
{
    ...
    public async Task<bool> SendQuoteEmailAsync(string toEmail, string clientName, string quoteNumber, byte[] pdfBytes, string publicToken)
    {
        try
        {
            ...
            var htmlBody = GetQuoteEmailHtml(clientName, quoteNumber, responseUrl, logoResource != null);
    ...
    public async Task<bool> SendReportEmailAsync(string toEmail, string userName, byte[] pdfBytes, DateTime startDate, DateTime endDate)
    {
        try
        {
            var from = _configuration["Email:From"];
            ...
            var periodText = $"{startDate:dd/MM/yyyy} - {endDate:dd/MM/yyyy}";

            using var message = new MailMessage();
            message.From = new MailAddress(from!, fromName);
            message.To.Add(new MailAddress(toEmail));
            message.Subject = $"Reporte Consolidado Finesoft — {periodText}";
            message.IsBodyHtml = true;
            ...
            var htmlBody = GetReportEmailHtml(userName, periodText, logoResource != null);
            var htmlView = AlternateView.CreateAlternateViewFromString(htmlBody, null, "text/html");
            ...
            using var pdfStream = new MemoryStream(pdfBytes);
            var pdfAttachment = new Attachment(pdfStream, $"Reporte_Finesoft_{startDate:yyyyMMdd}-{endDate:yyyyMMdd}.pdf", "application/pdf");
            message.Attachments.Add(pdfAttachment);
            ...
            _logger.LogInformation("✅ Reporte enviado a {Email} — Período {Period}", toEmail, periodText);
            return true;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "❌ Error al enviar reporte a {Email}", toEmail);
            return false;
        }
    }
    ...
    private string GetQuoteEmailHtml(string clientName, string quoteNumber, string responseUrl, bool hasLogo)
    {
...
<body style=""margin:0;padding:0;background:#f4f4f5;font-family:'DM Sans',Helvetica,Arial,sans-serif;"">
  <table width=""100%"" cellpadding=""0"" cellspacing=""0"" style=""background:#f4f4f5;padding:36px 16px;"">
        ...
        <table width=""500"" cellpadding=""0"" cellspacing=""0"" style=""background:#ffffff;border-radius:14px;overflow:hidden;border:1px solid #e4e4e7;box-shadow:0 1px 4px rgba(0,0,0,0.06);"">
          ...
          <tr>
            <td style=""padding:28px 32px;"">
              ...
              <p style=""margin:0 0 22px;font-size:14px;color:#71717a;line-height:1.65;"">Adjunto encontrarás el PDF con todos los detalles de tu cotización. Haz clic abajo para revisarla y responder.</p>
              <table width=""100%"" cellpadding=""0"" cellspacing=""0"" style=""background:#fafafa;border:1px solid #e4e4e7;border-radius:8px;margin-bottom:24px;"">
                <tr>
                  <td style=""padding:12px 18px;""><span style=""font-size:13px;color:#71717a;"">📎 <strong style=""color:#3f3f46;"">Cotizacion-{quoteNumber}.pdf</strong> adjunto a este correo</span></td>
                </tr>
              </table>
              <table cellpadding=""0"" cellspacing=""0"" width=""100%"">
                <tr>
                  <td align=""center"">
                    <a href=""{responseUrl}"" style=""display:inline-block;padding:13px 40px;background:#7c3aed;color:#fff;font-size:14px;font-weight:600;text-decoration:none;border-radius:8px;letter-spacing:0.2px;"">Ver y responder cotización →</a>
                  </td>
                </tr>
              </table>
              <p style=""margin:18px 0 0;font-size:12px;color:#a1a1aa;text-align:center;"">¿Dudas? Responde este correo directamente</p>
            </td>
          </tr>
          <tr>
            <td style=""padding:16px 32px;border-top:1px solid #f0f0f0;background:#fafafa;"">
              <table width=""100%"" cellpadding=""0"" cellspacing=""0"">
                <tr>
                  ...
                  <td align=""right""><a href=""mailto:informes@finesoft.com.mx"" style=""font-size:12px;color:#7c3aed;text-decoration:none;"">informes@finesoft.com.mx</a></td>
                </tr>
              </table>
            </td>
          </tr>
    ...
    private string GetReportEmailHtml(string userName, string periodText, bool hasLogo)
    {
        var logoHtml = hasLogo
            ? @"<img src=""cid:logo-finesoft"" alt=""Finesoft"" style=""max-width:140px;height:auto;display:block;"">"
            : @"<span style=""font-family:'DM Mono',monospace;font-size:15px;font-weight:500;color:#7c3aed;letter-spacing:0.5px;"">FINESOFT</span>";
      ...
      <td align=""center"">
        <table width=""500"" cellpadding=""0"" cellspacing=""0"" style=""background:#ffffff;border-radius:14px;overflow:hidden;border:1px solid #e4e4e7;box-shadow:0 1px 4px rgba(0,0,0,0.06);"">
            ...
            <td style=""padding:22px 32px;border-bottom:1px solid #f0f0f0;"">
              <table width=""100%"" cellpadding=""0"" cellspacing=""0"">
                <tr>
                  <td>{logoHtml}</td>
                  <td align=""right"">
                    <span style=""background:#f4f4f5;color:#71717a;font-size:11px;font-weight:600;padding:4px 10px;border-radius:20px;letter-spacing:0.5px;font-family:'DM Mono',monospace;"">REPORTE</span>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style=""padding:28px 32px;"">
              <p style=""margin:0 0 4px;font-size:13px;color:#71717a;"">Hola,</p>
              <p style=""margin:0 0 24px;font-size:21px;font-weight:600;color:#18181b;"">{userName}</p>
              <table width=""100%"" cellpadding=""0"" cellspacing=""0"" style=""background:#fafafa;border:1px solid #e4e4e7;border-radius:10px;margin-bottom:22px;"">
                <tr>
                  <td style=""padding:14px 20px;"">
                    <span style=""font-size:11px;font-weight:600;color:#a1a1aa;letter-spacing:1px;text-transform:uppercase;display:block;margin-bottom:4px;"">Período del reporte</span>
                    <span style=""font-family:'DM Mono',monospace;font-size:19px;font-weight:500;color:#18181b;"">{periodText}</span>
                  </td>
                </tr>
              </table>
              <p style=""margin:0 0 22px;font-size:14px;color:#71717a;line-height:1.65;"">Adjunto encontrarás el reporte consolidado de Finesoft con el resumen ejecutivo, métricas financieras, rendimiento de proyectos y más.</p>
              <table width=""100%"" cellpadding=""0"" cellspacing=""0"" style=""background:#fafafa;border:1px solid #e4e4e7;border-radius:8px;margin-bottom:24px;"">
                <tr>
                  <td style=""padding:12px 18px;""><span style=""font-size:13px;color:#71717a;"">📎 <strong style=""color:#3f3f46;"">Reporte_Finesoft.pdf</strong> adjunto a este correo</span></td>
                </tr>
              </table>
              <p style=""margin:0 0 12px;font-size:14px;color:#71717a;line-height:1.65;"">Este reporte incluye:</p>
              <ul style=""margin:0 0 22px 20px;color:#71717a;font-size:14px;line-height:1.8;"">
                <li>Resumen ejecutivo con KPIs principales</li>
                <li>Estado financiero (facturado, pagado, pendiente)</li>
                <li>Métricas de rendimiento de tickets</li>
                <li>Top clientes por facturación</li>
                <li>Estado de proyectos y horas registradas</li>
