// fs-front/Services/InvoiceApiService.cs  — COMPLETO ACTUALIZADO
...
public class InvoiceApiService : IInvoiceApiService
{
    ...
    public async Task<List<InvoiceDetailDto>?> GetInvoicesAsync(string? status = null,
    ...
    public async Task<PaginatedResponseDto<InvoiceDetailDto>?> GetInvoicesPaginatedAsync(
        string? search = null,
        string? status = null,
        ...
        bool sortDescending = false)
    {
        try
        {
            ...
            return await _httpClient.GetFromJsonAsync<PaginatedResponseDto<InvoiceDetailDto>>($"api/invoices{query}");
        }
        catch (Exception e)
        {
            Console.WriteLine($"Error al obtener facturas paginadas: {e.Message}");
            return null;
        }
    }
    ...
    public async Task<(bool Success, InvoiceDetailDto? CreatedInvoice, string? ErrorMessage)>
        CreateInvoiceAsync(InvoiceDto invoice)
    {
        try
        {
            ...
            return (false, null, ExtractErrorMessage(errorContent));
        }
        catch (Exception e) { return (false, null, $"Error de conexión: {e.Message}"); }
    }
        ...
        CreateInvoiceFromQuoteAsync(CreateInvoiceFromQuoteDto dto)
    {
        ...
        catch (Exception e) { return (false, null, $"Error: {e.Message}"); }
    }
    ...
    public async Task<(bool Success, string? ErrorMessage)> ChangeInvoiceStatusAsync(
    ...
    public async Task<(bool Success, InvoicePaymentDto? AddedPayment, string? ErrorMessage)>
        AddPaymentAsync(int invoiceId, InvoicePaymentDto payment)
    {
        try
        {
    ...
    public async Task<List<MonthlyClientSummaryDto>?> GetMonthlySummaryAsync()
    {
        try
        {
            return await _httpClient.GetFromJsonAsync<List<MonthlyClientSummaryDto>>(
                "api/invoices/monthly-summary");
        }
        catch (Exception e)
        {
            Console.WriteLine($"Error al obtener resumen mensual: {e.Message}");
            return null;
        }
    }
